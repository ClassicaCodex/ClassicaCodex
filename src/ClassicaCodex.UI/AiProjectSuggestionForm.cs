...
public sealed class AiProjectSuggestionForm : ScaledForm
{
    ...
    private async Task<string> BuildCorpusSampleAsync()
    {
        ...
        foreach(var node in await _nodes.GetByEditionAsync(edition.EditionId,true)){if(string.IsNullOrWhiteSpace(node.Text))continue;var key=$"P{n+1:00000}";var line=$"[{key}] [{PassageCitation.Display(node.CitationRef, node.Milestone)}] {node.Text}\n";if(text.Length+line.Length>MaxCorpusChars)break;n++;text.Append(line);_passages[key]=(node,edition);}return text.ToString();
    }
