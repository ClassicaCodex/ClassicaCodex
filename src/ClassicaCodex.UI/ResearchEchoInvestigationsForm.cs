...
public sealed class ResearchEchoInvestigationsForm : ScaledForm
{
    ...
    private async Task LoadResultsAsync(long selectId = 0)
    {
        ...
        _details.Text = $"{investigation.Method} • source {_authorName}, {_work.Title} {PassageCitation.Display(investigation.SourceCitationRef)}\n" +
    ...
    private async Task PromoteAsync()
    {
        ...
        var evidence = new EvidenceItem
        {
            ...
            Title = $"Echo candidate: {_work.Title} {PassageCitation.Display(investigation.SourceCitationRef)} ↔ {result.TargetWorkTitle} {PassageCitation.Display(result.TargetCitationRef)}",
            ...
            Excerpt = $"SOURCE — {_authorName}, {_work.Title} [{PassageCitation.Display(investigation.SourceCitationRef)}]\r\n{investigation.SourceText}\r\n\r\nTARGET — {result.TargetAuthorName}, {result.TargetWorkTitle} [{PassageCitation.Display(result.TargetCitationRef)}]\r\n{result.TargetText}",
