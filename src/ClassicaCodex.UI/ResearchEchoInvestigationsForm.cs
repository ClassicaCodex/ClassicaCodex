...
public sealed class ResearchEchoInvestigationsForm : Form
{
    ...
    public ResearchEchoInvestigationsForm(ResearchProject project, Work work, string authorName)
    {
        ...
        _results.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Rationale", HeaderText = "Rationale", Width = 280 });
        _results.SelectionChanged += (_, _) => ShowResult();
        _results.CellDoubleClick += async (_, _) => await OpenTargetAsync();
        ...
        review.Controls.AddRange([dispositionLabel, _disposition, save, promote, source, target, _note]);
    ...
    private async Task LoadResultsAsync(long selectId = 0)
    {
        ...
        _details.Text = $"{investigation.Method} • source {_authorName}, {_work.Title} {investigation.SourceCitationRef}\n" +
    ...
    private async Task PromoteAsync()
    {
        ...
        var evidence = new EvidenceItem
        {
            ...
            Title = $"Echo candidate: {_work.Title} {investigation.SourceCitationRef} ↔ {result.TargetWorkTitle} {result.TargetCitationRef}",
            ...
            Excerpt = $"SOURCE — {_authorName}, {_work.Title} [{investigation.SourceCitationRef}]\r\n{investigation.SourceText}\r\n\r\nTARGET — {result.TargetAuthorName}, {result.TargetWorkTitle} [{result.TargetCitationRef}]\r\n{result.TargetText}",
            Judgment = EvidenceJudgment.Uncertain,
            Relationship = EvidenceRelationship.Contextualizes,
            ResearcherNote = result.ResearcherNote,
